...
final class FrameProvider {
    ...
    -> CGRect
    {
        ...
        if whetherShowWeekView {
            return CGRect(origin: CGPoint(x: x, y: y), size: daySize)
        } else {
            return CGRect(origin: CGPoint(x: x, y: y), size: .init(width: daySize.width, height: 0))
        }
    }
    ...
    private var whetherShowWeekView: Bool {
        content.whetherShowWeekView
    }
    ...
    private func heightOfDaysOfTheWeekRowInMonth() -> CGFloat {
        if whetherShowWeekView {
            return monthsLayout.pinDaysOfWeekToTop ? 0 : (daySize.height + content.verticalDayMargin)
        } else {
            return 0
        }
    }
